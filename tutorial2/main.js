...
const race = {
    ...
    racers: [
        new Racer("Frankenstein"),
        new Racer("Buffalo"),
        new Racer("The Reckoning"),
        new Racer("Monster")
    ],
    textToType: quotes[util.randomNumber(0, quotes.length - 1)],
...
if (race.active) {
        if (!playerRacer.completedRace) playerRacer.updateStats(race.time, race.textToType);

        let racerInfoContainer = document.getElementById("racerInfoContainer");
        racerInfoContainer.innerHTML = "";

        // Sort the racers array by who is in the lead/finished the race first
        race.racers.sort((a, b) => {
            // If both the racers have finished the race then sort them by who finished first
            if (a.typingIndex >= race.textToType.length && b.typingIndex >= race.textToType.length) {
                if (a.raceFinishTime >= b.raceFinishTime) return 1;
            }

            // Sort by who is farther along
            if (a.typingIndex <= b.typingIndex) return 1;

            // Default return
            return -1;
        }).forEach((racer) => {
            if (!racer.completedRace) {
                racer.typingIndex += Math.round(racer.charactersPerSecond * (Math.random() * 0.30 + 0.70));
                racer.wordsPerMinute = racer.getWordsPerMinute(race.time);

                if (racer.typingIndex >= race.textToType.length) {
                    racer.typingIndex = race.textToType.length;
                    racer.finishRace(race.time, race.textToType);
                }
            }

            let element = `<li>${racer.name} - ${racer.wordsPerMinute} WPM`;

            if (racer.completedRace) {
                element += ` - <b>Finished in ${racer.raceFinishTime} seconds</b>`;
            }

            racerInfoContainer.innerHTML += element;
        });
    }
}, 1000);
